...
int check_res(A_DATATYPE  *A, B_DATATYPE *B,size_t Len){
    for(size_t i = 0;i<Len;i++){
        if(abs(A[i]-B[i])>1e-5){
            std::cout<<"wrong!!!!"<<std::endl;
            std::cout<<"npu res: "<<A[i]<<" cpu res: "<<B[i]<<std::endl;
            return 1;
        }
    }
...
void matrix_multiply_cpu(A_DATATYPE  *A, B_DATATYPE *B, C_DATATYPE *C, int M, int N, int K) {
    std::vector<C_DATATYPE> C2(M*N);  
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            C_DATATYPE sum = (C_DATATYPE)0.0;
            for (int k = 0; k < K; k++) {
                sum += A[i * K + k] * B[k * N + j];
            }
            C2[i * N + j] = sum;
...
int main(int argc, const char *argv[]) {
  ...
  for (int i = 0; i < A_VOLUME; i++) {
    AVec[i] = (std::bfloat16_t)0.1;//matmul_common::random_bfloat16_t();
  ...
  for (int i = 0; i < B_VOLUME; i++) {
    BVec[i] = (std::bfloat16_t)0.1;//matmul_common::random_bfloat16_t();
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    if (do_verify) {
      ...
      matrix_multiply_cpu(AVec.data(), BVec.data(), CVec.data(),  M,  N, K);
